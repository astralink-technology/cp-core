define("phone/getPhoneGrid", ['jquery', 'core/pager'],
    function ($, pager) {
        var _settings = null;
        ...
        var methods = {
            init: function (options) {
                ...
                $element.html(
                    [
                        '<table class="table table-bordered" id="phone-table' + settings.prefix + '">',
                            '<thead>',
                                '<tr>',
                                    '<td>Phone ID</td>',
                                    '<td>Country Code</td>',
                ...
                methods._getPhone(settings.prefix, _pageSize, _skip);
            },
            ...
            _pagerOnChange: function(prefix, currentActive){
                ...
                methods._getPhone(_settings.prefix, _pageSize, _skip);
            },
            _getPhone : function(prefix, pageSize, skip){
                        ...
                        if (!json.error){
                            ...
                            if (json.totalRowsAvailable > 0){
                                for (i = 0; i < json.totalRowsAvailable; i++){
                                    var phoneData = json.data[i];
                                    tableEntries += "<tr>" +
                                        "<td>" + phoneData.phoneId + "</td>" +
                                ...
                                tableEntries += "<tr>" +
                                    "<td colspan='7'>No Phone Entries</td>"
                                "</tr>"
                            }
                            $("#phone-table" + prefix + " tbody").html(tableEntries);
                        }
        ...
        $.fn.getPhoneGrid = function (methodOrOptions) {
